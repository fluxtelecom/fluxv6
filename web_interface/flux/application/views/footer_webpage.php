  ...
  <?php
	...
	if($result->num_rows() > 0){
  	...
  	if($footer != '' && ($this->session->userdata['logintype'] == 2 || $this->session->userdata['logintype'] ==-1)){ ?>
			...
			<a href="http://flux.net.br" target="_blank">
				<span style="color: #216397;text-shadow: 0px 1px 1px #FFF;">
					<strong>FluxSBC</strong>
				</span>
			</a>
	    </div>
	...
	<?php	} else  if($this->session->userdata['logintype'] ==0 || $this->session->userdata['logintype'] == 1){
		...
		<?} else {  ?>
	    <div class="col-md-offset-4 col-md-4">Copyright @ <?php echo date("Y"); ?> <a style="color:#3989c0;" href="http://flux.net.br" target="_blank"> Flux Telecom</a>. All Rights Reserved.
