...
// #############################################################################
class Fluxsbc_model extends CI_Model
{
    ...
    function get_fluxsbc_api_list($flag, $start = 0, $limit = 0)
    {
        $this->db_model->build_search('fluxsbc_api_list_search');
        if ($flag) {
            $query = $this->db_model->select("*", "flux_api", "", "id", "desc", $limit, $start);
        } else {
            $query = $this->db_model->countQuery("*", "flux_api", "");
        }
        return $query;
    }
