...
// ##############################################################################
class Fluxsbc extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library("fluxsbc_form");
        $this->load->library('flux/form', 'fluxsbc_form');
        $this->load->library('flux/permission');
        $this->load->library('freeswitch_lib');
        $this->load->library('FLUX_Sms');
        $this->load->model('fluxsbc_model');
    ...
    function fssipdevices_add($type = "")
    {
        ...
        if ($account_data['type'] == '-1' || $account_data['type'] == '1') {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->fsdevice_form_fields_for_customer($type), '');
            $this->load->view('view_fluxsbc_add_edit', $data);
        } else {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_freeswith_form_fields(), '');
            $this->load->view('view_fluxsbc_customer_add_edit', $data);
        }
    }
    ...
    function customer_fssipdevices_add($accountid)
    {
        ...
        $account_data = $this->session->userdata("accountinfo");
        $data['page_title'] = gettext('Create SIP Device');
        $data['form'] = $this->form->build_form($this->fluxsbc_form->fsdevice_form_fields_for_customer($accountid), "");
        if ($account_data['type'] == '-1' || $account_data['type'] == '1') {
            $this->load->view('view_fluxsbc_add_edit', $data);
        } else {
            $this->load->view('view_fluxsbc_customer_add_edit', $data);
    ...
    function fssipdevices_edit($edit_id = '')
    {
        ...
        $account = $this->fluxsbc_model->get_edited_data($edit_id);
        ...
        if ($account_data['type'] == '-1') {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_freeswith_form_fields($edit_id, $reseller_id), $account);
            $this->load->view('view_fluxsbc_add_edit', $data);
        } else {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_freeswith_form_fields($edit_id), $account);
            $this->load->view('view_fluxsbc_customer_add_edit', $data);
        }
    }
    ...
    function customer_fssipdevices_edit($edit_id, $accountid)
    {
        ...
        $account = $this->fluxsbc_model->get_edited_data($edit_id);
        if ($account_data['type'] == '-1' || $account_data['type'] == '1') {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->fsdevice_form_fields_for_customer($accountid, $edit_id), $account);
            $this->load->view('view_fluxsbc_add_edit', $data);
        } else {
    ...
    function fssipdevices_save($user_flg = false)
    {
        ...
        if (! $user_flg) {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_freeswith_form_fields($add_array['id']), $add_array);
        } else {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->fsdevice_form_fields_for_customer($add_array["accountcode"], $add_array['id']), $add_array);
        }
        if ($add_array['id'] != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->fluxsbc_model->edit_freeswith($add_array, $add_array['id']);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->fluxsbc_model->add_freeswith($add_array);
                echo json_encode(array(
    ...
    function customer_fssipdevices_save($user_flg = false)
    {
        $add_array = $this->input->post();
        if (! $user_flg) {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_freeswith_form_fields(), $add_array);
        } else {
            $data['form'] = $this->form->build_form($this->fluxsbc_form->fsdevice_form_fields_for_customer($add_array["accountcode"], $add_array['id']), $add_array);
        }
        if ($add_array['id'] != '') {
    ...
    function fssipdevices()
    {
        ...
        $this->session->set_userdata('advance_search', 0);
        $data['grid_fields'] = $this->fluxsbc_form->build_system_list_for_admin();
        $data["grid_buttons"] = $this->fluxsbc_form->build_grid_buttons();
        $data['form_search'] = $this->form->build_serach_form($this->fluxsbc_form->get_sipdevice_search_form());
        $this->load->view('view_fluxsbc_sip_devices_list', $data);
    }

    function fssipdevices_json()
    {
        $json_data = array();
        $count_all = $this->fluxsbc_model->fs_retrieve_sip_user(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $query = $this->fluxsbc_model->fs_retrieve_sip_user(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        ...
        $new_array = array();
        $id= 0;
        $response = $this->fluxsbc_model->reload_freeswitch($command,$id);
        if($response != ""){
    ...
    { 
        $sip_routing_status = $this->db_model->countQuery ( "*", "addons", array("package_name"=>"pbx") );  
    ...
    {
        $json_data = array();
        $count_all = $this->fluxsbc_model->get_sipdevices_list(false, $accountid, $entity_type);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $devices_result = array();
        $query = $this->fluxsbc_model->get_sipdevices_list(true, $accountid, $entity_type, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
    ...
    {
        $this->fluxsbc_model->delete_freeswith_devices($id);
    ...
    function livecall_report_json()
    {
        ...
        $response = $this->fluxsbc_model->reload_live_freeswitch($command);
        $calls = array();
        $calls_final = array();
        $data_header = array();
        $k = 0;
    ...
    function fsgateway()
    {
        ...
        $data['search_flag'] = true;
        $this->session->set_userdata('advance_search', 0);
        $data['grid_fields'] = $this->fluxsbc_form->build_fsgateway_list_for_admin();
        $data["grid_buttons"] = $this->fluxsbc_form->build_fdgateway_grid_buttons();
        $data['form_search'] = $this->form->build_serach_form($this->fluxsbc_form->get_gateway_search_form());
    ...
    function fsgateway_json()
    {
        ...
        $count_all = $this->fluxsbc_model->get_gateway_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $gateway_data = array();
        $query = $this->fluxsbc_model->get_gateway_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        ...
        $grid_fields = json_decode($this->fluxsbc_form->build_fsgateway_list_for_admin());
    ...
    function fsgateway_add()
    {
        ...
        $data['form'] = $this->form->build_form($this->fluxsbc_form->get_gateway_form_fields(), '');
    ...
    function fsgateway_edit($edit_id = '')
    {
        ...
        $data['form'] = $this->form->build_form($this->fluxsbc_form->get_gateway_form_fields(), $gateway_result);
        $this->load->view('view_fsgateway_add', $data);
    }
    ...
    function fsgateway_save()
    {
        ...
        $data['form'] = $this->form->build_form($this->fluxsbc_form->get_gateway_form_fields(), $gateway_data);
        $insert_arr = array();
        $gateway_arr = array();
        ...
        if ($gateway_data['id'] != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (isset($gateway_data['name'])) {
                    $query = $this->fluxsbc_model->check_unique_gateway_name('edit', 'name', $gateway_data['name']);
                ...
                if ($update) {
                    ...
                    $this->fluxsbc_model->reload_freeswitch($cmd, $sip_ip);
                    $cmd2 = "api sofia profile " . $sip_profile_info['name'] . " rescan reloadacl reloadxml";
                    $this->fluxsbc_model->reload_freeswitch($cmd2, $sip_ip);
                    // Kinjal FLUXUPDATE-943 Start
                    $this->fluxsbc_model->reload_freeswitch($mod_sofia);
                    // Kinjal FLUXUPDATE-943 END
                }
                // Kinjal FLUXUPDATE-943 Start
                sleep(1);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (isset($gateway_data['name'])) {
                    $query = $this->fluxsbc_model->check_unique_gateway_name('add', 'name', $gateway_data['name']);
                    if ($query > 0) {
                        echo json_encode(array(
                            "name_error" => gettext("Gateways already exist in system.")
                        ));
    ...
    function fsgateway_delete_multiple()
    {
        ...
        foreach ($gateway_info as $key => $value) {
            ...
            $this->fluxsbc_model->reload_freeswitch($cmd, $sip_profile_arr[$value['sip_profile_id']]['sip_ip']);
        }
        $this->db->where($where);
    ...
    function fssipprofile()
    {
        ...
        $data['search_flag'] = true;
        $this->session->set_userdata('advance_search', 0);
        $data['grid_fields'] = $this->fluxsbc_form->build_fssipprofile_list_for_admin();
        $data["grid_buttons"] = $this->fluxsbc_form->build_fssipprofile_grid_buttons();
        $data['form_search'] = $this->form->build_serach_form($this->fluxsbc_form->get_sipprofile_search_form());
    ...
    {
        $json_data = array();
        $count_all = $this->fluxsbc_model->get_sipprofile_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $gateway_data = array();
        $query = $this->fluxsbc_model->get_sipprofile_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->fluxsbc_form->build_fssipprofile_list_for_admin());
    ...
    function fssipprofile_action($button_name, $id)
    {
        ...
        $this->fluxsbc_model->reload_freeswitch($cmd);
    ...
    function fssipprofile_add($add = '')
    {
        ...
        if ($add == 'add') {
            ...
            if ($query > 0) {
                ...
                $sipprofile_data['id'] = '';
                $check_authentication = $this->fluxsbc_model->profile_authentication($sipprofile_data);
    ...
    function fssipprofile_edit($edit_id = '', $type = '', $name_prams = '')
    {
        ...
        $data['grid_fields'] = $this->fluxsbc_form->build_fssipprofile_params_list_for_admin();
    ...
    function fluxsbc_api_add($type = "")
    {
         $data['username'] = $this->session->userdata('user_name');
         $data['flag'] = 'create';
         $data['page_title'] = gettext('Create API Endpoint');
         $data['form'] = $this->form->build_form($this->fluxsbc_form->get_fluxsbc_api_form_fields(), '');
         $this->load->view('view_fluxsbc_api_add_edit', $data);
     }

    function fluxsbc_api_edit($edit_id = '')
    {
        ...
        if ($accountinfo['type'] == - 1 || $accountinfo['type'] == 2) {
            ...
            $account = $this->db_model->getSelect("*", "flux_api", $where);
            foreach ($account->result_array() as $key => $value) {
                $edit_data = $value;
            }
            $data['form'] = $this->form->build_form($this->fluxsbc_form->get_fluxsbc_api_form_fields($edit_id), $edit_data);
            $this->load->view('view_fluxsbc_api_add_edit', $data);
        } else {
            $this->session->set_flashdata('flux_notification', gettext('Permission Denied!'));
            redirect(base_url() . 'fluxsbc/fluxsbc_api_list/');
            exit();
        }
    }
    
    function fluxsbc_api_save()
    {
        $add_array = $this->input->post();
        $data['form'] = $this->form->build_form($this->fluxsbc_form->get_fluxsbc_api_form_fields($add_array['id']), $add_array);
        if ($add_array['id'] != '') {
            $data['page_title'] = gettext('Edit API Endpoints');
            if ($this->form_validation->run() == FALSE) {
                $data['validation_errors'] = validation_errors();
                echo $data['validation_errors'];
                exit();
            } else {
                $this->fluxsbc_model->edit_fluxsbc_api($add_array, $add_array['id']);
                echo json_encode(array(
                    "SUCCESS" => $add_array["api_name"] .' '. gettext("API Updated Successfully!")
                ));
                exit();
            }
        } else {
            $data['page_title'] = gettext('API Endpoint Details');
            if ($this->form_validation->run() == FALSE) {
                $data['validation_errors'] = validation_errors();
                echo $data['validation_errors'];
                exit();
            } else {
                $this->fluxsbc_model->add_fluxsbc_api($add_array);
                echo json_encode(array(
                    "SUCCESS" => $add_array["api_name"] .' '. gettext("API Added Successfully!")
                ));
                exit();
            }
        }
    ...
    function fluxsbc_api_list()
    {
     $data['username'] = $this->session->userdata('user_name');
     $data['page_title'] = gettext('Flux API');
     $data['search_flag'] = true;
     $this->session->set_userdata('advance_search', 0);
     $data['grid_fields'] = $this->fluxsbc_form->build_fluxsbc_api_list_for_admin();
     $data["grid_buttons"] = $this->fluxsbc_form->build_fluxsbc_api_grid_buttons();
     $data['form_search'] = $this->form->build_serach_form($this->fluxsbc_form->get_fluxsbc_api_search_form());
     $data['button_name'] = "Add API";
     $this->load->view('view_fluxsbc_api_list', $data);
 }
 
    function fluxsbc_api_list_json()
    {
        $json_data = array();
        $count_all = $this->fluxsbc_model->get_fluxsbc_api_list(false);
        $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
        $json_data = $paging_data["json_paging"];
        $gateway_data = array();
        $query = $this->fluxsbc_model->get_fluxsbc_api_list(true, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        $grid_fields = json_decode($this->fluxsbc_form->build_fluxsbc_api_list_for_admin());
        $json_data['rows'] = $this->form->build_grid($query, $grid_fields);
        echo json_encode($json_data);
    }
