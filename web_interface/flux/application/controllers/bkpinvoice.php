...
class ProcessInvoice extends MX_Controller
{
	...
	ini_set("max_execution_time", "259200");
        ...
        $this->fp = fopen("/var/log/flux/monitor.log", "a+");
          ...
          $this->CurrentDate = gmdate("2023-02-16 00:00:01");
        	$this->custom_current_date = gmdate("2023-02-16 23:59:59");
...
/*	$this->StartDate = gmdate("Y-m-01 00:00:01");
        ...
        $invoiceconf = $this->common->Get_Invoice_configuration($accountinfo);
       	if($invoiceconf != ''){
			...
			if($amount->num_rows > 0){
				...
				$this->db->update("accounts",array("balance" => $accountinfo['credit_limit']));
			}
			return $invoiceid;
		}
	}
	...
	$this->PrintLogger($this->Error_flag,$billable_calls_qr);
        ...
        if ($billable_calls->num_rows() > 0) {
            ...
            $account_limit_info = $this->db_model->getSelect("credit_limit", "accounts", array("id"=>$accountinfo["id"]));
 	    $base_currency = Common_model::$global_config ['system_config'] ['base_currency'];
		...
		if($secondsleft<10)
		    ...
		    'account_currency'=>$account_currency_info['currency'],
                ...
                $tempArr = array(
                            ...
                            "reseller_id" => $accountinfo['reseller_id'],
                            "order_item_id" => "0",
                            "description" => "Invoice sem Cobrança",
                            "debit" => 0,
                            "charge_type" => "Invoice sem Cobrança",
                            "created_date" => $this->EndDate,
        		    'base_currency'=>$base_currency,
        		    'exchange_rate'=>$account_currency_info['currencyrate'],
        		    'account_currency'=>$account_currency_info['currency'],
		...
		foreach($renewable_order as $orderkey => $ordervalue){
		  ...
		  if (!empty($user_product_info) && $product_data->num_rows() > 0) {
				...
				//	$account_balance = $accountdata ['posttoexternal'] == 1 ? $accountdata ['credit_limit'] - ($accountdata ['balance']) : $accountdata ['balance'];
					...
					$account_balance = $accountdata ['balance'] + $accountdata ['credit_limit'];
					}
...
"failover_call_type"=>1,"always_vm_flag"=>1,"user_busy_vm_flag"=>1,"user_not_registered_vm_flag"=>1,"no_answer_vm_flag"=>1,"failover_extensions"=>"");
					if($product_info['status'] == 1 ||  $ordervalue['billing_type'] == 0){
						...
						$add_array=array(
								...
								'subject'=>'Produto configurado para cobrança One Time. Conta: '.$accountdata['number'],
						...
						$add_array_details=array(
								...
								'message'=>'Produto configurado para cobrança One Time. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
								'attachment'=>'',
								'creation_date'=>gmdate("Y-m-d H:i:s"),
								'status'=>0,
								);
					...
					if(($product_info['can_purchase'] == 1 ||$product_info['can_resell'] == 1) && ($product_info['reseller_id'] == $ordervalue['reseller_id'] && $ordervalue['reseller_id'] > 0)){   
						...
						$add_array=array(
								...
								'accountid'=>$accountdata['id'],
								'reseller_id'=>$accountdata['reseller_id'],
								'subject'=>'Produto nao encontrado na conta de revenda. Conta: '.$accountdata['number'],
								'creation_date'=>gmdate("Y-m-d H:i:s"),
								'last_modified_date'=>gmdate("Y-m-d H:i:s"),
								'department_id'=>1,
								'support_ticket_number'=>$support_ticket_number,
						...
						$add_array_details=array(
								'support_ticket_id'=>$support_ticket_id,
								'generate_account_id'=>$accountdata['id'],
								'message'=>'Produto nao encontrado na conta de revenda. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
								'attachment'=>'',
								'creation_date'=>gmdate("Y-m-d H:i:s"),
								'status'=>0,
								);
					 ...
					 $accountnumber = $account_info['number'];
				            $where_ticket_number = array("name" => "ticket_digits");
										...
										$add_array=array(
												...
												'subject'=>'Produto desativao. Conta: '.$account_data['number'],
												'creation_date'=>gmdate("Y-m-d H:i:s"),
												'last_modified_date'=>gmdate("Y-m-d H:i:s"),
												'department_id'=>1,
												'support_ticket_number'=>$support_ticket_number,
										...
										$add_array_details=array(
												...
												'message'=>'Produto desativado. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
												'attachment'=>'',
												'creation_date'=>gmdate("Y-m-d H:i:s"),
												'status'=>0,
												);
						...
						$add_array=array(
								...
								'subject'=>'Produto desativado - '.$account_info['number'],
								'creation_date'=>gmdate("Y-m-d H:i:s"),
								'last_modified_date'=>gmdate("Y-m-d H:i:s"),
								'department_id'=>1,
								'support_ticket_number'=>$support_ticket_number,
