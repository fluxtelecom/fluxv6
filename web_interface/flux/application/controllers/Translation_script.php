        ...
        if ($query->num_rows () > 0) {
            ...
            if ($languages->num_rows () > 0) {
				...
				fclose($myfile);
		                ...
		                $this->session->set_flashdata ( 'flux_errormsg', 'unable to write files.' );
			    }
                        }

                $this->session->set_flashdata ( 'flux_errormsg', 'Language Translations sucessfull!' );
                redirect ( base_url () . 'systems/languages_list/' );
                die();
            } else {
                $this->session->set_flashdata ( 'flux_notification', 'No data found!' );
                redirect ( base_url () . 'systems/languages_list/' );
                die();
            }
        } else {
