...
class GenerateInvoice extends MX_Controller {
	...
	function count_invoice_data($account, $start_date = "", $end_date = "") {
		...
		$inv_data_query = "select count(id) as count,sum(debit) as debit,sum(credit) as credit from invoice_details where accountid=" . $account ['id'] . " AND created_date >='" . $start_date . "' AND created_date <= '" . $end_date . "'  AND invoiceid=0 AND item_type != 'Gratuita'";
