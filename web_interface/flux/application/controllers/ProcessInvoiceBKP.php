...
//	       $this->custom_current_date = gmdate("Y-m-d 23:59:59");
      $this->custom_current_date = "2023-03-02 00:00:01";
      $this->CurrentDate = "2023-03-02 00:00:01";
    }
...
//				$this->common->mail_to_users("new_invoice",$final_array);
				$this->PrintLogger($this->Error_flag,$final_array);
...
order_items.reseller_id,order_items.accountid,order_items.setup_fee,order_items.price',array("order_items.next_billing_date <=" => $this->custom_current_date,"order_items.product_category <>"=>"3", "order_items.is_terminated" => "0","orders.payment_status <>"=>"FAIL"),'order_items','orders.id=order_items.order_id', 'inner', '' ,'','','');
	...
	if($renewable_order->num_rows > 0){
		...
		foreach($renewable_order as $orderkey => $ordervalue){
		 ...
		 $product_data = $this->db_model->getSelect("*", "products", array("id"=>$ordervalue['product_id'],'is_deleted'=>"0",'status'=>"0"));
...
"failover_call_type"=>1,"always_vm_flag"=>1,"user_busy_vm_flag"=>1,"user_not_registered_vm_flag"=>1,"no_answer_vm_flag"=>1,"failover_extensions"=>"");
					...
					
					{
						...
						$where_ticket_number = array("name" => "ticket_digits");
											...
											$add_array=array(
													...
													'subject'=>'Pedido billing_type One Time. Conta: '.$accountdata['number'],
													'creation_date'=>gmdate("Y-m-d H:i:s"),
													'last_modified_date'=>gmdate("Y-m-d H:i:s"),
													'department_id'=>1,
													'support_ticket_number'=>$support_ticket_number,
													'status'=>0,
													'close_ticket_display_flag' => 1,
													);
											$this->db->insert("support_ticket", $add_array);
										 $support_ticket_id = $this->db->insert_id();
											$add_array_details=array(
													'support_ticket_id'=>$support_ticket_id,
													'generate_account_id'=>$accountdata['id'],
													'message'=>'Pedido Billing Type One Time. Funcao: renew_product. Pedido removido.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$product_info['id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
													'attachment'=>'',
													'creation_date'=>gmdate("Y-m-d H:i:s"),
													'status'=>0,
													);
					...
					{   
						...
						$where_ticket_number = array("name" => "ticket_digits");
																	...
																	$add_array=array(
																			...
																			'accountid'=>$accountdata['id'],
																			'reseller_id'=>$accountdata['reseller_id'],
																			'subject'=>'Produto nao pode ser comprado. Conta: '.$accountdata['number'],
																			'creation_date'=>gmdate("Y-m-d H:i:s"),
																			'last_modified_date'=>gmdate("Y-m-d H:i:s"),
																			'department_id'=>1,
																			'support_ticket_number'=>$support_ticket_number,
																 ...
																 $support_ticket_id = $this->db->insert_id();
																	$add_array_details=array(
																			'support_ticket_id'=>$support_ticket_id,
																			'generate_account_id'=>$accountdata['id'],
																			'message'=>'Produto nao pode ser comprado. Funcao: renew_product. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$product_info['id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
																			'attachment'=>'',
																			'creation_date'=>gmdate("Y-m-d H:i:s"),
																			'status'=>0,
																			);
	   ...
	   $where_ticket_number = array("name" => "ticket_digits");
				...
				$add_array=array(
						...
						'accountid'=>$accountdata['id'],
						'reseller_id'=>$accountdata['reseller_id'],
						'subject'=>'Produto removido falta de saldo. Conta: '.$accountdata['number'],
						'creation_date'=>gmdate("Y-m-d H:i:s"),
						'last_modified_date'=>gmdate("Y-m-d H:i:s"),
						'department_id'=>1,
						'support_ticket_number'=>$support_ticket_number,
			 ...
			 $support_ticket_id = $this->db->insert_id();
				$add_array_details=array(
						'support_ticket_id'=>$support_ticket_id,
						'generate_account_id'=>$accountdata['id'],
						'message'=>'Produto removido por falta de saldo. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
						'attachment'=>'',
						'creation_date'=>gmdate("Y-m-d H:i:s"),
						'status'=>0,
						);
...
//				    $product_info['add_invoice_credit']= "true";
    ...
    $this->common->mail_to_users ( "product_renewed", $final_array );
					...
					if($accountdata["reseller_id"] > 0 && $accountdata["is_distributor"] == 1  && $product_info['product_category'] != '4' && $is_apply_commission == "true"){ 
			...
			$add_array=array(
					...
					'accountid'=>$accountdata['id'],
					'reseller_id'=>$accountdata['reseller_id'],
					'subject'=>'Produto não encontrado. Conta: '.$accountdata['number'],
					'creation_date'=>gmdate("Y-m-d H:i:s"),
					'last_modified_date'=>gmdate("Y-m-d H:i:s"),
					'department_id'=>1,
					'support_ticket_number'=>$support_ticket_number,
		 ...
		 $support_ticket_id = $this->db->insert_id();
			$add_array_details=array(
					'support_ticket_id'=>$support_ticket_id,
					'generate_account_id'=>$accountdata['id'],
					'message'=>'Produto não encontrado na conta do usuário. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
					'attachment'=>'',
					'creation_date'=>gmdate("Y-m-d H:i:s"),
					'status'=>0,
					);
		 ...
		 /*$update_order_arr = array("is_terminated"=>'1',
		  ...
		  $where_ticket_number = array("name" => "ticket_digits");
				...
				$add_array=array(
						...
						'accountid'=>$accountdata['id'],
						'reseller_id'=>$accountdata['reseller_id'],
						'subject'=>'Produto desativado. Conta: '.$accountdata['number'],
						'creation_date'=>gmdate("Y-m-d H:i:s"),
						'last_modified_date'=>gmdate("Y-m-d H:i:s"),
						'department_id'=>1,
						'support_ticket_number'=>$support_ticket_number,
			 ...
			 $support_ticket_id = $this->db->insert_id();
				$add_array_details=array(
						'support_ticket_id'=>$support_ticket_id,
						'generate_account_id'=>$accountdata['id'],
						'message'=>'Produto desativado no pedido do usuário. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$ordervalue['product_id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
						'attachment'=>'',
						'creation_date'=>gmdate("Y-m-d H:i:s"),
						'status'=>0,
						);
...
/*	else {
	  $where_ticket_number = array("name" => "ticket_digits");
		...
		$add_array=array(
				...
				'accountid'=>$accountdata['id'],
				'reseller_id'=>$accountdata['reseller_id'],
				'subject'=>'Pedido não encontrado. Conta: '.$accountdata['number'],
				'creation_date'=>gmdate("Y-m-d H:i:s"),
				'last_modified_date'=>gmdate("Y-m-d H:i:s"),
				'department_id'=>1,
				'support_ticket_number'=>$support_ticket_number,
	...
	if($renewable_order->num_rows > 0){
		...
		foreach($renewable_order as $orderkey => $ordervalue){
			...
			$product_info = $this->db_model->getSelect("name,status,price,can_purchase,billing_days,billing_type,can_resell,reseller_id","products",array("id"=>$ordervalue['product_id'],"status"=>"0"));
			if($product_info->num_rows > 0){
				...
				$account_info = $this->db_model->getSelect("*","accounts",array("id"=>$ordervalue['accountid'],"reseller_id"=> $reseller_id,"status"=>0,"deleted"=>0));
				...
				if($account_data->num_rows > 0){ 
					...
					if($product_info['status'] == 0   ||  $product_info['billing_type'] == 0 ){ 
						...
						$where_ticket_number = array("name" => "ticket_digits");
								...
								$add_array=array(
										...
										'subject'=>'Pedido desativado. Conta: '.$account_info['number'],
										'creation_date'=>gmdate("Y-m-d H:i:s"),
										'last_modified_date'=>gmdate("Y-m-d H:i:s"),
										'department_id'=>1,
										'support_ticket_number'=>$support_ticket_number,
							 ...
							 $support_ticket_id = $this->db->insert_id();
								$add_array_details=array(
										'support_ticket_id'=>$support_ticket_id,
										'generate_account_id'=>$account_info['id'],
										'message'=>'Produto desativado. Funcao: product_renewal_reminder. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_number'].'<br> ID Produto: '.$product_info['id'].'<br> ID Pedido: '.$ordervalue['order_id'].'',
										'attachment'=>'',
										'creation_date'=>gmdate("Y-m-d H:i:s"),
										'status'=>0,
										);
     ...
     $this->common->mail_to_users ( "product_renewal_notice", $final_array );
			     ...
			     $where_ticket_number = array("name" => "ticket_digits");
								...
								$add_array=array(
										...
										'accountid'=>$account_info['id'],
										'reseller_id'=>$account_info['reseller_id'],
										'subject'=>'Produto desativado. Conta: '.$account_info['number'],
										'creation_date'=>gmdate("Y-m-d H:i:s"),
										'last_modified_date'=>gmdate("Y-m-d H:i:s"),
										'department_id'=>1,
										'support_ticket_number'=>$support_ticket_number,
								...
								$add_array_details=array(
										'support_ticket_id'=>$support_ticket_id,
										'generate_account_id'=>$account_info['id'],
										'message'=>'Produto desativado. Funcao: product_renewal_reminder product_info. Invoice nao criado.<br> Número do Pedido: '.$ordervalue['order_id'].'<br> ID Pedido: '.$ordervalue['id'].'',
										'attachment'=>'',
										'creation_date'=>gmdate("Y-m-d H:i:s"),
										'status'=>0,
										);
		  ...
		  /* $accountid = $ordervalue['accountid'];
		                              $account_data = $this->db_model->getSelect("*","accounts",array("id"=>$accountid,"status"=>0,"deleted"=>0));
		                              $account_info = $account_data->result_array()[0];
		                              $accountnumber = $account_info['number'];
		  			     			$where_ticket_number = array("name" => "ticket_digits");
		  			     			$ticket_number = $this->db_model->Select("value", "system", $where_ticket_number, "", "", "", "");
		  			     			$ticket_digits = $ticket_number->result_array();
		  			     			$ticket_digits = $ticket_digits[0]['value'];
		  			     			...
		  			     			$add_array=array(
		  			     					...
		  			     					'accountid'=>$account_info['id'],
		  			     					'reseller_id'=>$account_info['reseller_id'],
		  			     					'subject'=>'Produto não encontrado - '.$account_info['number'],
		  			     					'creation_date'=>gmdate("Y-m-d H:i:s"),
		  			     					'last_modified_date'=>gmdate("Y-m-d H:i:s"),
		  			     					'department_id'=>8,
		  			     					'support_ticket_number'=>$support_ticket_number,
