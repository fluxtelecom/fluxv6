...
//*######################################################################
//
//	CRON : 0 0 * * * cd /var/www/html/flux/cron/ && php cron.php MailchimpSync
//
//*######################################################################
...
class Mailchimpsync extends CI_Controller {
	...
	function __construct() {		
		...
		if(file_exists(Common_model::$global_config ['system_config'] ['log_path']."flux-mailchimp.log"))
		$this->fp = fopen(Common_model::$global_config ['system_config'] ['log_path']."flux-mailchimp.log", "a+");
	}
		...
		if ($query->num_rows () > 0) {
			...
			foreach ( $account_data as $data_key => $accountinfo ) {
				...
				if ($accountinfo['email'] != '')
				{
					...
					//Call mailchimp api to insert flux contacts to mailchimp
