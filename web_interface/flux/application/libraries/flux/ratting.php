...
class Ratting {
    ...
    function ratting_list($data,$account_data){
		...
		if($account_data['type']==1 && $balance > $account_data['balance']){
			$this->CI->session->set_flashdata('flux_notification', 'Reseller balance is low!');
			redirect(base_url() . 'web2fax/web2fax_list/');
			exit;
		}	
		if($balance <= 0){
			$this->CI->session->set_flashdata('flux_notification', 'Insuficient belance!');
			redirect(base_url() . 'web2fax/web2fax_list/');
			exit;
		}
		...
		if($termination_cost >= $origination_cost){
			$this->CI->session->set_flashdata('flux_notification', 'Cost not correct!');
			redirect(base_url() . 'web2fax/web2fax_list/');
			exit;
		}
		...
		$this->CI->session->set_flashdata('flux_notification', 'Rategroup not found!');
		redirect(base_url() . 'web2fax/web2fax_list/');
		exit;
	}
	return true;
