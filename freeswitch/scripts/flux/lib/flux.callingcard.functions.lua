	...
	if(config['cc_ani_auth'] == "0")  then 
		...
		if(aniinfo ~= nil) then
		  ...
		  if(tonumber(aniinfo['status']) == 1) then
			session:streamFile("flux-callerid-blocked.wav")      
			session:hangup();
		  end   
          ...
          if (card_flag==1) then
              error_xml_without_cdr("","ACCOUNT_EXPIRE","FLUX-CALLINGCARD",config['playback_audio_notification'])
		      session:hangup();
		  end
		  ani_flag = 1
		end 
	...
	if (ani_flag == 0) then
		...
		if(cardnum == -1) then
			...
			while (tonumber(authenticated) ~= 1 and tonumber(retries) < tonumber(config['card_retries'])) do
				cardnum = session:playAndGetDigits(1, 15, 1, config['calling_cards_number_input_timeout'], "#", "flux-accountnum.wav", "", "^[0-9]+$")
				Logger.debug("Got DTMF digits: ".. cardnum )
			     if (cardnum ~= "") then		    
		        		cardinfo = get_account(cardnum);       
		    	     end
			     if(cardinfo == nil) then
					session:streamFile("flux-badaccount.wav")      
	    		     else 
					authenticated = 1
					pin = cardinfo['pin']
		    	     end
			 ...
			 if ( tonumber(authenticated) ~= 1 and tonumber(retries) == tonumber(config['card_retries']) ) then
				session:streamFile("flux-goodbye.wav")      
				session:hangup();
		        end
		end
		if ( pin ~= "" ) then
			...
			while (tonumber(authenticated) ~= 1 and tonumber(retries) < tonumber(config['pin_retries'])) do
				...
				local pin_number = session:playAndGetDigits(1, 15, 1, config['calling_cards_pin_input_timeout'], "#", "flux-pleasepin.wav", "", "^[0-9]+$")
				Logger.debug("We recieved a pin : ".. pin_number )

				if ( pin_number ~= pin ) then	
						session:streamFile("flux-invalidpin.wav")      
				else 
						authenticated = 1
				end
				retries = retries + 1
			...
			if ( tonumber(authenticated) ~= 1 and tonumber(retries) == tonumber(config['pin_retries']) ) then
				session:streamFile("flux-invalidpin")      
				session:hangup();
			end
...
function save_ani(cardinfo)
	local result = session:playAndGetDigits(1, 1, 1, 5000, "#", "flux-register.wav", "", "^[1-1]+$")
...
function playback_ivr(userinfo)   
	...
	while (tonumber(retries) < tonumber(config['ivr_count'])) do
		result = session:playAndGetDigits(1, 1, 2, config['calling_cards_number_input_timeout'], "#", "flux-callingcard-menu.wav", "", "^[1-3]+$")
...
function say_balance(cardinfo)
	...
	if ( balance > 0 ) then
		if (tonumber(config['calling_cards_balance_announce']) == tonumber('0')) then
			session:streamFile( "flux-this-card-has-a-balance-of.wav" )
...
function play_amount(amount)
    ...
    session:streamFile( "flux-point.wav" )
...
function say_cost(numberinfo,customer_carddata)        
    if ( tonumber(config['calling_cards_rate_announce']) == tonumber('0') ) then
        ...
        if ( tonumber(numberinfo['connectcost']) > 0 ) then
            ...
            session:streamFile( "flux-willapply.wav" );
        end
    end
end
...
function say_timelimit(minutes) 
	if ( session:ready() ) then
	   ...
	   Logger.debug("[SAY_TIMELIMIT] MINUTES "..tonumber(minutes));    
		...
		session:streamFile( "flux-this-call-will-last.wav" );
		if ( minutes == 1 ) then
		    session:execute(  "say", "en number pronounced " .. minutes );
		    session:streamFile( "flux-minute.wav" );
	    ...
	    elseif ( minutes < 1 ) then
		session:streamFile( "flux-not-enough-credit.wav" );
		session:streamFile( "flux-goodbye.wav" );
		session:hangup();
	    end
	end 
...
function dialout( original_destination_number, destination_number, maxlength, userinfo, user_rates, origination_dp_string ,number_loop_str,parentinfo,livecall_reseller)
	if ( session:ready() ) then
     ...
     --                ---------------------------------------------------------------------- 
					...
					-- end
				    -- FLUXUPDATE-651(Record Calling card calls)
		        ...
		        error_xml_without_cdr(destination_number,"TERMINATION_RATE_NOT_FOUND","FLUX-CALLINGCARD",config['playback_audio_notification']) 
		    	return
		    end
	       end
        end
...
function process_destination(userinfo)
	...
	local destination = session:playAndGetDigits(1, 35, 3, config['calling_cards_dial_input_timeout'], "#", "flux-phonenum.wav", "flux-badphone.wav", '^[0-9]+$')
 ...
 --        Logger.info("[DIALPLAN] Caller ID Translation Ends")
    ...
    if(block_prefix == "false") then
	   		error_xml_without_cdr(destination,"DESTINATION_BLOCKED","FLUX-CALLINGCARD",config['playback_audio_notification'],userinfo['id']) ;
	        return 
    end	
	...
	if (tonumber(balance) <= 0 and tonumber(package_id) == 0) then
		error_xml_without_cdr('CALLINGCARD',"NO_SUFFICIENT_FUND","FLUX-CALLINGCARD",config['playback_audio_notification']) 
		session:streamFile( "flux-goodbye.wav" );
		session:hangup();
	end
    ...
    if( tmp_array == 'NO_SUFFICIENT_FUND' or tmp_array == 'ORIGNATION_RATE_NOT_FOUND') then
	    error_xml_without_cdr(destination,tmp_array,"FLUX-CALLINGCARD",config['playback_audio_notification'],userinfo['id']) 
	    return
	end
	...
	while (tonumber(userinfo['reseller_id']) > 0 and tonumber(maxlength) > 0 ) do 
        ...
        if (reseller_userinfo == nil) then
	    	    error_xml_without_cdr(destination,"ACCOUNT_INACTIVE_DELETED","FLUX-CALLINGCARD",config['playback_audio_notification']); 
		        return
		else
		    ...
		    if( block_prefix == false) then
			    ...
			    error_xml_without_cdr(destination,"DESTINATION_BLOCKED","FLUX-CALLINGCARD",config['playback_audio_notification']) ;
	            return
		    end
            ...
            if( tmp_array_reseller == 'NO_SUFFICIENT_FUND' or tmp_array_reseller == 'ORIGNATION_RATE_NOT_FOUND') then
        	    error_xml_without_cdr(destination,tmp_array_reseller,"FLUX-CALLINGCARD",1,reseller_userinfo['id']) 
        	    return
        	end
		    ...
		    if (userinfo['loss_less_routing'] == 1 and (tonumber(reseller_maxlength) < 1 or tonumber(reseller_rates['cost']) > tonumber(user_rates['cost']))) then
			    error_xml_without_cdr(destination,"RESELLER_COST_CHEAP","FLUX-CALLINGCARD",config['playback_audio_notification']); 
