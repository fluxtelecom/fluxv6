...
function sip_device_fail_over(didinfo,xml,destination_number)
	...
	if(SipDestinationInfo and SipDestinationInfo ~= '')then
		...
		if(sip_routing_arr)then
			...
			table.insert(xml, [[<action application="set" data="variable_sip_contact_port=]]..params:getHeader("variable_sip_contact_port")..[["/>]]);
...
--HP 10-Jan-2019 PBX Feature Code Related changes.
	...
	-- if(sip_routing_arr and tonumber(sip_routing_arr['do_not_disturb']) == 1)then
		...
		if(tonumber(sip_routing_arr['call_forwarding_flag']) == 0 and sip_routing_arr['call_forwarding_destination'] ~= nil and sip_routing_arr['call_forwarding_destination'] ~= '')then
			...
			bridge = "{sip_invite_params=user=LOCAL,sip_from_uri="..sip_routing_arr['call_forwarding_destination'].."@${domain_name}}[leg_timeout="..config['leg_timeout'].."]user/"..sip_routing_arr['call_forwarding_destination'].."@"..params:getHeader("variable_sip_to_host")..":"..params:getHeader("variable_sip_contact_port")
			...
			sip_call_string = "user/"..destination_number.."@"..params:getHeader("variable_sip_to_host")..":"..params:getHeader("variable_sip_contact_port")
...
function custom_inbound_10(xml,didinfo,userinfo,config,xml_did_rates,callerid_array,livecall_data)
	...
	common_chan_var = "{sip_invite_params=user=LOCAL,sip_from_uri="..didinfo['extensions'].."@${domain_name}}"
		for i = 1, #destination_str do
			...
			bridge_str = bridge_str.."[leg_timeout="..didinfo['leg_timeout'].."]user/"..destination_str[i].."@"..params:getHeader("variable_sip_to_host")..":"..params:getHeader("variable_sip_contact_port") --//HP: 
			if i <= #deli_str then
