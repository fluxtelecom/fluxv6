...
#############################################################################################
# FluxSBC Version 6 Beta
#
#  UPDATES: 2022-10-20
#
# 1) Compila o FreeSwitch da fonte em vez de usar o repositório SignalWire
#    que requer uma conta SignalWire com um token anexado a essa conta
#
# 2) Usa MariaDB do repositório Debian em vez de MySQL de seu repositório.
#
# 3) Não usa RemiRepo para PHP 7.3+.
#
# 4) Opção para instalar o Postfix ao invés do sendmail que proporciona um melhor registro
#
# 6) IPTables e Fail2ban opcionais
#
...
TEMP_USER_ANSWER="no"
FLUX_SOURCE_DIR=/opt/FluxSBC
FLUX_HOST_DOMAIN_NAME="host.domain.tld"
os_codename=$(lsb_release -cs)


#FluxSBC Configuration
...
echo "**************** INTRODUCTION *******************"
echo ""
echo "This is an alternative to the Flux Telecom FluxSBC installer."
...
echo "will take much longer than the Flux Telecom installer, but without the need for third party packages,"
...
echo "Fully Qualified Domain Name (FQDN)/hosname already set (e.g. flux.myserver.com) before continuing beyond this point."
echo ""
echo "If you have not done so, the FQDN/hostname can be easily set, as an example, with the command: "
echo "hostnamectl set-hostname flux.yourhostname.com"
echo ""
...
get_user_response ()
{
        ...
        echo ""
        echo ""
        read -p "Enter aFQDN example (i.e ${FLUX_HOST_DOMAIN_NAME}), or an IP Address for this server: "
...
cp -f "${FLUX_SOURCE_DIR}/freeswitch/sounds/"*.wav "${FS_SOUNDSDIR}"/
cp -f "${FLUX_SOURCE_DIR}/freeswitch/conf/autoload_configs/"* /etc/freeswitch/autoload_configs/


#Creating freeswitch user
groupadd freeswitch
adduser --quiet --system --home /usr/share/freeswitch --gecos 'FreeSWITCH' --ingroup freeswitch freeswitch --disabled-password 
chown -R freeswitch:freeswitch /usr/share/freeswitch/
chmod -R ug=rwX,o= /usr/share/freeswitch/
chmod -R u=rwx,g=rx /usr/share/freeswitch/bin/*
...
; service
Type=forking
PIDFile=/usr/share/freeswitch/run/freeswitch.pid
Environment="DAEMON_OPTS=-nonat"
Environment="USER=freeswitch"
Environment="GROUP=freeswitch"
EnvironmentFile=-/etc/default/freeswitch
ExecStartPre=/bin/chown -R ${USER}:${GROUP} /usr/share/freeswitch
ExecStart=/usr/share/freeswitch/bin/freeswitch -u ${USER} -g ${GROUP} -ncwait ${DAEMON_OPTS}
TimeoutSec=45s
Restart=always
...
#Fetch FluxSBC Source
get_fluxsbc_source()
{
        cd /opt
        git clone -b v6.0 https://github.com/fluxtelecom/fluxsbc.git flux
} #end get_fluxsbc_source
...
cp -rf ${FLUX_SOURCE_DIR}/web_interface/nginx/deb_fluxsbc.conf /etc/nginx/conf.d/flux.conf
...
#Make changes for FluxSBC to use php7.4-fpm in Debian 11 
...
printf '%s\n'  'g/^/s|fastcgi_pass unix:/var/run/php/php7.3-fpm.sock;|fastcgi_pass unix:/var/run/php/php7.4-fpm.sock;|g' 'w' 'q' | ed -s /etc/nginx/conf.d/flux.conf
fi

systemctl restart nginx
} #end normalize_fluxsbc
...
install_database ()
{
        ...
        mysql -uroot -p${MYSQL_ROOT_PASSWORD} -e "ALTER USER 'fluxuser'@'localhost' IDENTIFIED BY '${FLUXUSER_MYSQL_PASSWORD}';"
        ...
        printf '%s\n' 'g/^/s|\(/\*!50003 SET collation_connection  = \)utf8mb4_0900_ai_ci\( \*/ ;\)|\1utf8mb4_unicode_520_ci\2|g' 'w' 'q' | ed -s ${FLUX_SOURCE_DIR}/database/flux-5.0.sql
        printf '%s\n' 'g/^/s|\(/\*!50001 SET collation_connection      = \)utf8mb4_0900_ai_ci\( \*/;\)|\1utf8mb4_unicode_520_ci\2|g' 'w' 'q' | ed -s ${FLUX_SOURCE_DIR}/database/flux-5.0.sql

        mysql -uroot -p${MYSQL_ROOT_PASSWORD} flux < ${FLUX_SOURCE_DIR}/database/flux-5.0.sql
        mysql -uroot -p${MYSQL_ROOT_PASSWORD} flux < ${FLUX_SOURCE_DIR}/database/flux-5.0.1.sql
...
start_installation ()
{
	...
	MYSQL_ROOT_PASSWORD=`echo "$(genpasswd 20)" | sed s/./*/5`
        ...
        echo "******************************************************************************************"
        echo "**********                                                                      **********"
        echo "**********           Your FluxSBC is installed successfully                       **********"
